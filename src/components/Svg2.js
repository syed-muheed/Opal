import React from 'react'

const Svg2 = () => {
  return (
    <div className='bg-[#0a0a0a] text-[#595959] pt-[15vh] px-[2vh] md:hidden'>
        <div className='text-[13px]'>
            <div className='text-[40px] text-[white] font-thin
            h-[130px] w-[150px] m-auto leading-[50px]'>Image & Sound.</div>
            <div className='bg-[#333232] h-[1px] w-full'></div>
            <div className=' pt-[10vh]'>
                        <img className='w-[100%]' src='/icon/svg4.png'/>
                    </div>
            <div className=''>
                <h1 className='text-[33px] text-[white] font-thin  flex h-[10vh] items-center'>Lens</h1>
                <div className='flex justify-between items-center  h-[6vh]'>
                    <div>DFOV</div>
                    <div>70º</div>
                </div>
                <div className='flex justify-between items-center h-[6vh] border-b-[1px] border-[#333232] border-t-[1px] '>
                    <div>EFL</div>
                    <div className=' '>4.8 .mm</div>
                </div>
                <div className='flex justify-between items-center h-[6vh]'>
                    <div>Aperture</div>
                    <div>ƒ1.8</div>
                </div>
                <div className='flex justify-between items-center h-[6vh] border-y-[1px] border-[#333232]'>
                    <div>Focal range</div>
                    <div>10 cm → ∞</div>
                </div>
                <div className='flex justify-between items-center h-[6vh] border-y-[1px] border-[#333232]'>
                    <div>Auto focus type</div>
                    <div>Phase detection</div>
                </div>
                <div className='flex justify-between items-center h-[6vh] border-y-[1px] border-[#333232]'>
                    <div>Lens structure</div>
                    <div>6 plastic elements</div>
                </div>
                
            </div>
            
            <div className='pt-[3vh] pb-[6vh]'>
                <h1 className='text-[33px] text-[white] font-thin  flex h-[10vh] items-center'>Sensor</h1>
                <div className='flex justify-between items-center h-[6vh]'>
                    <div>Model</div>
                    <div>Sony IMX582 Exmor RS</div>
                </div>
                <div className='flex justify-between items-center h-[6vh] border-y-[1px] border-[#333232]'>
                    <div>Type</div>
                    <div>CMOS Rolling Shutter</div>
                </div>
                <div className='flex justify-between items-center h-[6vh]'>
                    <div>Size</div>
                    <div>12.7 mm</div>
                </div>
                <div className='flex justify-between items-center h-[6vh] border-y-[1px] border-[#333232]'>
                    <div>Pixel size</div>
                    <div>0.8 μm</div>
                </div>
                <div className='flex justify-between items-center h-[6vh] border-b-[1px] border-[#333232]'>
                    <div>Resolution</div>
                    <div>48MP binned to 1080P</div>
                </div>
                <div className='flex justify-between items-center h-[6vh] '>
                    <div>FPS</div>
                    <div>30</div>
                </div>
               <div className='flex flex-col justify-center gap-[4vh] h-[20vh]'>
                <div className='w-[120px] ml-[1vh]'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 132 17"><path fill="#4D4F53" d="M131.651.012s-2.374 9.741-3.326 14.337c-.103.483-.195.978-.333 1.45-.046.16-.242.414-.38.414-1.484.034-2.968.023-4.602.023.564-.288.978-.541 1.415-.702.449-.161.69-.483.782-.898.369-1.68.76-3.348 1.036-5.04.149-.874-.403-1.357-1.346-1.368-2.969-.012-5.949 0-8.917 0h-.771c.103-.553.195-1.047.299-1.53.126-.564.299-1.105.391-1.669.058-.368.219-.483.564-.472.84.012 1.691 0 2.531 0 2.589 0 5.178-.023 7.767.012.563.011.805-.196.851-.725.035-.334.081-.679.127-1.105h-.656c-3.878 0-7.744.012-11.621 0-1.726 0-1.968.426-2.508 1.876-.392 1.047-.507 2.197-.725 3.302-.046.207-.035.426-.035.633.012.874.426 1.3 1.3 1.3h8.86c.759 0 .805.023.644.782-.23 1.163-.518 2.313-.771 3.475-.092.414-.287.564-.748.564-3.256-.023-6.523 0-9.78-.023-.552 0-.782.184-.874.713-.058.31-.184.76-.414.863-.426.184-.863.012-1.22-.38-1.715-1.852-3.454-3.65-5.304-5.361-.127-.115-.173-.31-.253-.472.172-.057.334-.161.506-.161.84-.012 1.691-.012 2.531-.012 1.634 0 2.382-.598 2.716-2.197.195-.932.391-1.876.541-2.808.057-.38-.012-.794-.012-1.185 0-.598-.437-.76-.886-.874-.161-.046-.345-.023-.518-.023-3.969 0-7.939.011-11.908-.012-.564 0-.702.161-.783.725-.58 4.138-1.693 8.263-2.565 12.346-.07.345-.242.518-.633.506-.852-.034-1.715-.011-2.658-.011 1.075-5.376 2.315-10.71 3.601-16.04.023-.103.184-.207.31-.241.139-.046.3-.012.461-.012h37.314ZM34.943 16.236c-1.15 0-2.3.011-3.44-.012-.196 0-.403-.184-.564-.333a121.95 121.95 0 0 1-2.669-2.773c-.322-.345-.575-.426-1.024-.138-1.657 1.047-3.348 2.036-5.04 3.026-.23.138-.54.218-.805.218-6.133.012-12.277.012-18.41.012-.15 0-.299-.023-.529-.035.541-2.657 1.07-5.28 1.611-7.973H.046C.035 8.182.012 8.136 0 8.09c.725-.599 1-1.45 2.393-1.45H3.43c1.093 0 1.093 0 1.3-1.059.253-1.311.495-2.623.725-3.946.057-.334.184-.518.552-.518h15.36c.058 0 .104.035.161.046-.195.622-1.196 1.404-1.806 1.404-3.256 0-6.524 0-9.78.011-.748 0-.725.035-.84.76-.138.863-.357 1.714-.552 2.566-.046.195-.092.402-.161.713H19.5c0 1.27-2.474 1.642-3.323 1.61-2.508-.091-5.028-.022-7.536-.034-.449 0-.621.115-.702.587-.322 1.841-.713 3.67-1.07 5.511-.023.092 0 .184 0 .357h.621c4.488 0 8.975.012 13.462-.023.449 0 .932-.276 1.346-.518 1.128-.667 2.233-1.403 3.349-2.094.172-.103.38-.16.563-.264.138-.08.38-.207.369-.276-.035-.173-.173-.323-.3-.46-.345-.369-.701-.726-1.058-1.094-.506-.53-1.001-1.058-1.496-1.588-.483-.517-.955-1.035-1.565-1.702 1.116 0 2.06-.012 3.003.011.311.012.622.104.932.184.15.046.311.138.426.253.725.748 1.45 1.473 2.129 2.255.299.346.54.38.909.138a74.103 74.103 0 0 1 2.6-1.564c.602-.346 2.416-1.277 2.416-1.277l2.554.115-.701.506s-5.857 3.705-5.926 3.762c-.069.058-.046.368.035.449.736.725 1.518 1.415 2.255 2.152.782.794 1.53 1.622 2.29 2.439-.047.08-.093.126-.139.184ZM52.87 8.25h-3.924c-.16.84-.31 1.68-.483 2.52-.345 1.668-.702 3.325-1.047 4.982-.069.357-.241.495-.621.483a29.093 29.093 0 0 0-2.244 0c-.38.012-.483-.138-.403-.483.334-1.553.668-3.095.99-4.648.127-.576.196-1.174.345-1.738.092-.356.265-.69.437-1.139-1.242 0-2.439 0-3.635.012-.081 0-.22.15-.242.253-.242 1.116-.46 2.243-.713 3.36-.3 1.357-.599 2.715-.921 4.073-.023.115-.173.287-.265.287-1.024.023-2.06.012-3.175.012.092-.483.15-.886.23-1.289.518-2.508 1.024-5.016 1.553-7.513.184-.852.207-.852 1.082-.852h14.14c.173 0 .346 0 .518.012 1.346.08 1.795.76 1.496 2.07-.403 1.75-.725 3.522-1.093 5.27-.138.656-.265 1.312-.449 1.957-.034.138-.264.333-.414.333-.886.035-1.76.012-2.646.023-.392 0-.45-.207-.38-.517.253-1.151.518-2.313.77-3.464.127-.575.23-1.162.38-1.737.162-.575.38-1.128.564-1.692.07-.172.104-.356.15-.575Zm13.806-1.669c1.783 0 3.566-.011 5.35 0 1.369.012 1.887.69 1.6 2.037-.392 1.853-.645 3.74-1.175 5.546-.425 1.473-1.196 2.07-2.623 2.07-2.91.012-5.833 0-8.744 0-.69 0-1.381.012-2.071-.01-1.07-.035-1.6-.795-1.393-1.83.357-1.726.587-3.475 1.013-5.178.196-.76.679-1.495 1.162-2.128.437-.576 1.185-.518 1.864-.518 1.68.011 3.348 0 5.017.011Zm2.29 8.043c.436-2.117.862-4.223 1.3-6.374h-7.963c-.425 2.128-.84 4.223-1.277 6.374h7.94Z"></path><path fill="#4D4F53" d="M107.074 4.604c-.219 1.058-.449 2.117-.668 3.175-.08.38-.333.437-.678.437-1.876-.011-3.763-.011-5.638-.011h-.714c.046.138.058.196.092.23 1.692 1.91 3.372 3.843 5.086 5.73.472.53 1.059.955 1.576 1.45.15.138.253.31.449.563H96.281c.035-.276.035-.529.092-.782.345-1.565.702-3.13 1.036-4.706.276-1.288.495-2.589.794-3.877.172-.737.448-1.45.713-2.152.035-.08.219-.161.322-.161 2.555 0 5.097 0 7.652.011.034.012.092.058.184.093Zm-32.332 11.62c.426-1.99.817-3.843 1.208-5.684.196-.909.38-1.83.576-2.738.172-.794.563-1.14 1.38-1.185.633-.035 1.266-.035 1.899-.035h7.824c-.38.92-1.415 1.634-2.278 1.634-1.611 0-3.222.023-4.833-.012-.506-.011-.725.104-.805.656-.242 1.53-.46 3.084-.944 4.533a31.142 31.142 0 0 0-.702 2.509c-.034.126-.195.299-.299.299-.966.035-1.956.023-3.026.023Z"></path></svg>
                </div>
                <div className='w-[250px] text-[12px]'>
                Powered by premium mobile phone sensor technology for a true-to-life image quality.
                </div>
               </div>
                
            </div>
           
        </div>
    </div>
  )
}



export default Svg2